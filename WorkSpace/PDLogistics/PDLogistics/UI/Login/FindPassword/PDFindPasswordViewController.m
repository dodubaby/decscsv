...
- (void)viewDidLoad {
    ...
    [submitbtn handleControlEvents:UIControlEventTouchUpInside actionBlock:^(id sender) {
        ...
        if (phonetfd.text.length==0||codetfd.text.length==0||passwordtfd.text.length==0) {
            UIAlertView *alt=[[UIAlertView alloc] initWithTitle:@"请输入全部信息后再找回密码" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [alt show];
        }else{
            PDHTTPEngine *engine=[[PDHTTPEngine alloc] init];
            [engine registerWithphone:phonetfd.text verification:codetfd.text password:passwordtfd.text type:@"getpwd" success:^(AFHTTPRequestOperation *operation, id responseObject) {
                NSLog(@"responseObject==%@",responseObject);
